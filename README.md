...
| `storage/` | Archivos subidos vía API, persistidos por job (no se versiona). |
...
| `GET /comparisons/{job_id}` | Estado y resultado de un job (`pending` / `done` / `error`). |
...
3. `core.compare_documents()` orquesta, en orden: extracción de texto (`extraction.py`), descubrimiento de secciones esperadas a partir del propio `expected` (`structure.py`, con fallback a IA en `structure_ai.py`), diff semántico por bloques (`semantic.py`) que distingue cambios reales de rellenos de plantilla, clasificación de severidad por IA (`severity.py`) y, si está habilitado, un veredicto visual (`visual.py` + `core/ai/gemini_provider.py`).
4. El resultado (`ComparisonResult`) queda disponible vía `GET /comparisons/{job_id}`, serializado a JSON por `api/schemas.py`.
5. Cualquier error del núcleo (`CoreError` y subclases) se captura por job y se expone como `status: "error"`, sin tumbar el servicio ni otros jobs en curso.
...
- Los archivos subidos vía API ahora se **persisten** en `storage/comparisons/<job_id>/` en vez de un directorio temporal que se borraba al terminar el job.
- Esa persistencia habilitó tres endpoints nuevos: **listar** comparaciones con filtros/paginación, **descargar** los archivos originales de un job y **re-ejecutar** (`rerun`) una comparación sin volver a subir archivos.
- El CORS de la API pasó de un origen fijo (`localhost:5173`) a aceptar cualquier puerto de `localhost`/`127.0.0.1`, para el frontend de desarrollo local.
- `core/` se consolidó a partir de tres prototipos de análisis independientes (`hybrid/`, `semantic/`, `structured/`) que fueron eliminados; toda la lógica de extracción, estructura, semántica, severidad y visual vive ahora en un único núcleo desacoplado de FastAPI y de Playwright.
- Integración de archivo `Dockerfile` para instalación LibreOffice en el servicio
